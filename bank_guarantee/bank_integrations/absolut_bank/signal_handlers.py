...
from bank_guarantee.signals import get_ask_on_query, client_confirm_offer
...
def set_to_bank_after_ask_on_query(sender, request, user, **kwargs):
    if request.bank and request.bank.code == BankCode.CODE_ABSOLUT:
...
@receiver(client_confirm_offer)
def confirm_offer(sender, request, **kwargs):
    if request.bank and request.bank.code == BankCode.CODE_ABSOLUT:
        adapter = get_send_adapter_class(request=request)(
